...
const { exists } = require('../model/register');
...
route.post("/login", async (req, res) => {
  ...
  if (!user) {
    ...
    if (password == user[0]?.password) {
      res.send({ status: 200, data: user[0] });
    } else {
...
route.post("/profile", async (req, res) => {
  ...
  const _id = req.body._id; const profile = "Completed"
...
console.log(existUser)
  if (existUser.length > 0) {
    userData = { _id: { $in: existUser[0].sendBy }
    }
    const docs = await User.find(userData);
    if (docs) {
      res.send({ status: 200, data: docs });
    } else {
